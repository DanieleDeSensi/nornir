...
#define DEBUG(x)                                                               \
  do {                                                                         \
    std::cerr << "[Configuration] " << x << std::endl;                         \
  } while (0)
#define DEBUGB(x)                                                              \
  do {                                                                         \
    x;                                                                         \
...
Configuration::Configuration(const Parameters &p, uint numHMPs)
    : _numServiceNodes(0), _numHMPs(numHMPs), _p(p),
      _combinationsCreated(false) {
...
void Configuration::createAllRealCombinations() {
  if (_numHMPs > 1) {
    throw std::runtime_error(
        "createAllRealCombinations() cannot be used on HMP systems.");
  }
...
const std::vector<KnobsValues> &Configuration::getAllRealCombinations() const {
  if (!_combinationsCreated) {
    throw std::runtime_error(
        "[configuration.cpp] Combinations not created yet.");
  }
...
Knob *Configuration::getKnob(KnobType t) const {
  if (_numHMPs > 1) {
    throw std::runtime_error(
        "getKnob: When running on HMP please specify the HMPid.");
  }
  return getKnob(0, t);
}
...
double Configuration::getRealValue(KnobType t) const {
  if (_numHMPs > 1) {
    throw std::runtime_error(
        "Please use getRealValue(cpuId, t) when running on HMP.");
  }
...
bool Configuration::virtualCoresWillChange(const KnobsValues &values) const {
  ...
  for (size_t i = 0; i < _knobs.size(); i++) {
    if (real(i, KNOB_VIRTUAL_CORES) !=
        _knobs[i][KNOB_VIRTUAL_CORES]->getRealValue()) {
      return true;
    }
  }
  return false;
...
void Configuration::stopReconfigurationStatsKnob(ticks start, KnobType type,
  ...
  if (_p.statsReconfiguration) {
    if (type == KNOB_VIRTUAL_CORES && !vcChanged) {
      ...
      double ms =
          ticksToMilliseconds(getticks() - start, _p.archData.ticksPerNs);
...
void Configuration::stopReconfigurationStatsTotal(ticks start) {
  if (_p.statsReconfiguration) {
    double ms = ticksToMilliseconds(getticks() - start, _p.archData.ticksPerNs);
...
void Configuration::setValues(const KnobsValues &values) {
  ...
  for (size_t k = 0; k < _knobs.size(); k++) {
    for (size_t i = 0; i < KNOB_NUM; i++) {
      ...
      stopReconfigurationStatsKnob(reconfigurationStart, (KnobType) i,
                                   vcChanged);
    }
  }
...
ConfigurationFarm::ConfigurationFarm(const Parameters &p,
  ...
  for (size_t c = 0; c < _numHMPs; c++) {
    ...
    _knobs[c][KNOB_MAPPING] = new KnobMappingFarm(
        p, *dynamic_cast<KnobVirtualCoresFarm *>(_knobs[c][KNOB_VIRTUAL_CORES]),
        *dynamic_cast<KnobHyperThreading *>(_knobs[c][KNOB_HYPERTHREADING]),
        emitter, collector);
    _knobs[c][KNOB_FREQUENCY] = new KnobFrequency(
        p, *dynamic_cast<KnobMappingFarm *>(_knobs[c][KNOB_MAPPING]));
    if (p.clockModulationEmulated) {
      _knobs[c][KNOB_CLKMOD] = new KnobClkModEmulated(p);
    } else {
      _knobs[c][KNOB_CLKMOD] = new KnobClkMod(
  ...
  _triggers[TRIGGER_TYPE_Q_BLOCKING] =
      new TriggerQBlocking(p.triggerQBlocking, p.thresholdQBlocking,
                           p.thresholdQBlockingBelt, samples, emitter);
...
ConfigurationPipe::ConfigurationPipe(
    const Parameters &p, Smoother<MonitoredSample> const *samples,
