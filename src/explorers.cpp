...
Explorer::Explorer(std::vector<bool> knobs,
    ...
    : _knobs(knobs), _additionalPoints(additionalPoints) {
  ;
}
...
ExplorerLowDiscrepancy::ExplorerLowDiscrepancy(
    std::vector<bool> knobs, StrategyExploration explorationStrategy,
    std::vector<KnobsValues> additionalPoints)
    : Explorer(knobs, additionalPoints),
  ...
  DEBUG("[Explorator] Will generate low discrepancy points in "
        << d << " dimensions");
...
ExplorerMultiple::ExplorerMultiple(std::vector<bool> knobs, Explorer *explorer,
                                   KnobType kt, size_t numValues)
    : Explorer(knobs), _explorer(explorer), _kt(kt), _numValues(numValues),
      _nextValue(0), _lastkv(explorer->nextRelativeKnobsValues()) {
  ;
}
