...
static_assert(nornir::REQUIREMENT_NUM <= RIFF_MAX_CUSTOM_FIELDS,
              "Please increase RIFF_MAX_CUSTOM_FIELDS");
...
#define DEBUG(x)                                                               \
  do {                                                                         \
    std::cerr << "[Instrumenter] " << x << std::endl;                          \
  } while (0)
#define DEBUGB(x)                                                              \
  do {                                                                         \
    x                                                                          \
...
std::pair<nn::socket *, uint>
Instrumenter::connectPidChannel(const std::string &parametersFile,
                                uint pid) const {
...
std::pair<nn::socket *, uint>
Instrumenter::getChannel(const std::string &parametersFile,
...
Instrumenter::Instrumenter(const std::string &parametersFile, size_t numThreads,
                           riff::Aggregator *aggregator, bool startServer)
    : InstrumenterHelper(getChannel(parametersFile, startServer), numThreads,
                         aggregator) {
}
...
NornirInstrumenter *nornir_instrumenter_create(const char *parametersFile) {
  return reinterpret_cast<NornirInstrumenter *>(
      new nornir::Instrumenter(parametersFile));
}

NornirInstrumenter *
nornir_instrumenter_create_with_threads(const char *parametersFile,
                                        size_t numThreads) {
  return reinterpret_cast<NornirInstrumenter *>(
...
void nornir_instrumenter_begin_with_threads(NornirInstrumenter *instrumenter,
                                            size_t threadId) {
...
void nornir_instrumenter_end_with_threads(NornirInstrumenter *instrumenter,
                                          size_t threadId) {
...
unsigned long
nornir_instrumenter_get_execution_time(NornirInstrumenter *instrumenter) {
  return reinterpret_cast<nornir::Instrumenter *>(instrumenter)
      ->getExecutionTime();
}

unsigned long long
nornir_instrumenter_get_total_tasks(NornirInstrumenter *instrumenter) {
  return reinterpret_cast<nornir::Instrumenter *>(instrumenter)
      ->getTotalTasks();
}

void nornir_instrumenter_set_total_threads(NornirInstrumenter *instrumenter,
                                           unsigned int totalThreads) {
  return reinterpret_cast<nornir::Instrumenter *>(instrumenter)
      ->setTotalThreads(totalThreads);
}

void nornir_instrumenter_set_phase_id(NornirInstrumenter *instrumenter,
                                      unsigned int phaseId) {
  return reinterpret_cast<nornir::Instrumenter *>(instrumenter)
      ->setPhaseId(phaseId);
}

void nornir_instrumenter_mark_inconsistent_samples(
    NornirInstrumenter *instrumenter) {
  return reinterpret_cast<nornir::Instrumenter *>(instrumenter)
      ->markInconsistentSamples();
}
}
...
#define DEBUG(x)                                                               \
  do {                                                                         \
    cerr << "[External Server] " << x << endl << flush;                        \
  } while (0)
#define DEBUGB(x)                                                              \
  do {                                                                         \
    x;                                                                         \
...
static void managerCleanup(Manager *m,
...
void InstrumenterServer::run() {
  ...
  if (!mammut::utils::existsDirectory(getInstrumentationChannelsPath())) {
    if (mkdir(getInstrumentationChannelsPath().c_str(), ACCESSPERMS)) {
      throw std::runtime_error(
          "Impossible to create nornir instrumentation channels dir.");
    }
    if (system(
            (std::string("chmod ugo+rwx ") + getInstrumentationChannelsPath())
                .c_str()) == -1) {
      throw std::runtime_error(
          "Impossible to set permission to nornir channel dir.");
    }
  }
  ...
  std::list<ApplicationInstance *> instances;
  // We need to change the rights of the channel because most likely this
  // manager will be executed with sudoers rights (since we need to change
  // frequency etc..). By changing the rights we allow non sudoers users to
  // interact with the manager on this channel.
  if (system((std::string("chmod ugo+rwx ") +
              getInstrumentationConnectionChannelPath())
                 .c_str()) == -1) {
  ...
  */
  while (true) {
    ...
    if (system((std::string("chmod ugo+rwx ") +
                getInstrumentationPidChannelPath(pid))
                   .c_str()) == -1) {
      throw std::runtime_error(
          "Impossible to set permission to nornir channel.");
    }
    ...
    // Make a tmp directory and dump parameters in a file to let the Parameters
    // struct load them
    ...
    std::string tmpParamFileName = std::string(dir_name) + std::string("/") +
                                   std::string("parameters.xml");
