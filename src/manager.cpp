...
static std::vector<AdaptiveNode *>
convertWorkers(ff::svector<ff::ff_node *> w) {
...
Manager::Manager(Parameters nornirParameters)
    : _terminated(false), _p(validate(nornirParameters)), _counter(NULL),
      _task(NULL), _topology(NULL), _cpufreq(NULL), _samples(initSamples()),
      _variations(new MovingAverageExponential<double>(0.5)), _totalTasks(0),
      _remainingTasks(0), _deadline(0), _lastStoredSampleMs(0),
      _inhibited(false), _configuration(NULL), _selector(NULL), _pid(0),
      _toSimulate(false) {
  ...
  for (LoggerType lt : _p.loggersTypes) {
    ...
    default: { throw std::runtime_error("Unknown logger type."); }
    }
  }
...
void Manager::run() {
  ...
  if (!_toSimulate) {
    ThreadHandler *thisThread =
        _task->getProcessHandler(getpid())->getThreadHandler(gettid());
  ...
  if (isPrimaryRequirement(_p.requirements.executionTime)) {
    ...
    if (now / 1000.0 >= _deadline) {
      ...
      _p.requirements.throughput =
          _remainingTasks / ((_deadline * 1000.0 - now) / 1000.0);
    }
  }
}
...
bool Manager::persist() const {
  ...
  case STRATEGY_PERSISTENCE_VARIATION: {
    ...
    r = _samples->size() < 1 || variation.throughput > _p.persistenceValue ||
  ...
  if (_p.requirements.anySpecified()) {
    for (size_t c = 0; c < _numHMP; c++) {
      ...
      if (!_p.knobHyperthreadingEnabled) {
        _configuration->getKnob(c, KNOB_HYPERTHREADING)
            ->lock(_p.knobHyperthreadingFixedValue);
      }
  ...
  if (_toSimulate) {
    ...
    if (_terminated) {
      ...
      if (_p.synchronousWorkers) {
        ...
        for (size_t c = 0; c < _numHMP; c++) {
          numWorkers +=
              _configuration->getKnob(c, KNOB_VIRTUAL_CORES)->getRealValue();
        }
...
void Manager::logObservation() {
  for (auto logger : _p.loggers) {
    logger->log(_selector->isCalibrating(), *_configuration, *_samples,
                _p.requirements);
  }
}
...
ManagerInstrumented::ManagerInstrumented(nn::socket &riffSocket, int chid,
                                         Parameters nornirParameters)
...
void ManagerInstrumented::waitForStart() {
  ...
  for (size_t c = 0; c < _numHMP; c++) {
    if (_monitor.getTotalThreads()) {
      dynamic_cast<KnobVirtualCores *>(
          _configuration->getKnob(c, KNOB_VIRTUAL_CORES))
          ->changeMax(_monitor.getTotalThreads());
    }
    dynamic_cast<KnobMappingExternal *>(
        _configuration->getKnob(c, KNOB_MAPPING))
        ->setPid(_pid);
    if (_p.clockModulationEmulated) {
      dynamic_cast<KnobClkModEmulated *>(
          _configuration->getKnob(c, KNOB_CLKMOD))
          ->setPid(_pid);
    }
  }
}
...
MonitoredSample ManagerInstrumented::getSample() {
  ...
  if (sample.inconsistent) {
    if (_p.requirements.minUtilization != NORNIR_REQUIREMENT_UNDEF ||
      ...
      throw std::runtime_error(
          "You specified requirements on loadPercentage but instrumenter is "
  ...
  if (sample.inconsistent) {
    if (_p.requirements.latency != NORNIR_REQUIREMENT_UNDEF) {
      throw std::runtime_error(
          "You specified requirements on latency but instrumenter is "
  ...
  if (sample.customFields[REQUIREMENT_EXECUTION_TIME]) {
    _p.requirements.executionTime =
        sample.customFields[REQUIREMENT_EXECUTION_TIME];
  }
  ...
  if (sample.customFields[REQUIREMENT_TASKS_NUMBER]) {
    _p.requirements.expectedTasksNumber =
        sample.customFields[REQUIREMENT_TASKS_NUMBER];
  }

  if (sample.customFields[REQUIREMENT_UTILIZATION_MAX]) {
    _p.requirements.maxUtilization =
        sample.customFields[REQUIREMENT_UTILIZATION_MAX];
  }

  if (sample.customFields[REQUIREMENT_UTILIZATION_MIN]) {
    _p.requirements.minUtilization =
        sample.customFields[REQUIREMENT_UTILIZATION_MIN];
  }
...
ManagerBlackBox::ManagerBlackBox(pid_t pid, Parameters nornirParameters)
    : Manager(nornirParameters),
      _process(
          nornirParameters.mammut.getInstanceTask()->getProcessHandler(pid)) {
  Manager::_pid = pid;
  Manager::_configuration = new ConfigurationExternal(_p, _numHMP);
...
MonitoredSample ManagerBlackBox::getSample() {
  ...
  sample.throughput =
      instructions / ((getMillisecondsTime() - _lastStoredSampleMs) / 1000.0);
  sample.latency =
      sample.throughput;         // We set it only for phase detection purposes.
...
void initNodesPreRun(Parameters p, AdaptiveNode *emitter,
                     std::vector<AdaptiveNode *> workers,
                     AdaptiveNode *collector, volatile bool *terminated,
                     ff::ff_thread *lb, ff::ff_thread *gt) {
  ...
  if (collector) {
    collector->initPreRun(p, NODE_TYPE_COLLECTOR, terminated, gt);
  }
}

void initNodesPostRun(AdaptiveNode *emitter,
                      std::vector<AdaptiveNode *> workers,
...
void ManagerFastFlow::waitForStart() {
  ...
  DEBUG("Init pre run");
  initNodesPreRun(_p, _emitter, _activeWorkers, _collector, &_terminated,
                  _farm->getlb(), _farm->getgt());
...
MonitoredSample getSampleResponse(std::vector<AdaptiveNode *> nodes,
...
ManagerFastFlow::ManagerFastFlow(ff_farm<> *farm, Parameters parameters)
    : Manager(parameters), _farm(farm),
  ...
  Manager::_configuration =
      new ConfigurationFarm(_p, _samples, _emitter, _activeWorkers, _collector,
                            _farm->getgt(), &_terminated, _numHMP);
}
...
void ManagerFastFlow::postConfigurationManagement() {
  ...
  for (size_t c = 0; c < _numHMP; c++) {
    const KnobVirtualCoresFarm *knobWorkers =
        dynamic_cast<const KnobVirtualCoresFarm *>(
            _configuration->getKnob(c, KNOB_VIRTUAL_CORES));
...
ManagerFastFlowPipeline::ManagerFastFlowPipeline(ff_pipeline *pipe,
                                                 std::vector<bool> farmsFlags,
                                                 Parameters nornirParameters)
    : Manager(nornirParameters), _pipe(pipe), _farmsFlags(farmsFlags) {
  if (pipe->getStages().size() != _pipe->getStages().size()) {
    throw std::runtime_error(
        "You need to specify a flag for each node in the pipeline.");
  }
  Manager::_pid = getpid();
  // configuration creation, lockKnobs,
  // _configuration->createAllRealCombinations(); and _selector =
  // createSelector(); are delayed in the waitForStart(). We need indeed to run
  // the pipeline for a while to get the values of the knobPipeline knob.
}
...
static bool fixValidAllocation(std::vector<ff_farm<> *> farms,
...
static bool areEqualAllocations(std::vector<double> allocation_first,
                                std::vector<double> &allocation_second) {
...
void ManagerFastFlowPipeline::waitForStart() {
  ...
  for (size_t i = 0; i < _pipe->getStages().size(); i++) {
    if (_farmsFlags[i]) {
      ...
      AdaptiveNode *emitter =
          dynamic_cast<AdaptiveNode *>(realFarm->getEmitter());
      AdaptiveNode *collector =
          dynamic_cast<AdaptiveNode *>(realFarm->getCollector());
      std::vector<AdaptiveNode *> workers =
          convertWorkers(realFarm->getWorkers());
      ...
      KnobVirtualCoresFarm *f = new KnobVirtualCoresFarm(
          _p, emitter, collector, realFarm->getgt(), workers, &_terminated);
  ...
  for (auto k : farmsKnobs) {
    ...
    k->setRealValue(1);
    // When a change in the number of workers in a farm occurs, their workers
    // push a sample, so we need to retrieve it and discard it
    for (auto an : allworkers) {
      MonitoredSample tmp;
      an->getSampleResponse(tmp, 0);
    }
    allworkers = k->getActiveWorkers();
    _activeWorkers.insert(_activeWorkers.end(), allworkers.begin(),
                          allworkers.end());
  ...
  while (numWorkers < allowedTotalWorkers && !finished) {
    ...
    monitoredData[bottleneckId].throughput =
        monitoredData[bottleneckId].throughput *
    ...
    if (!fixValidAllocation(farms, tmp)) {
      ...
      if (!_allowedValues.empty() &&
          areEqualAllocations(tmp, _allowedValues.back())) {
  ...
  Manager::_configuration =
      new ConfigurationPipe(_p, _samples, farmsKnobs, _allowedValues, _numHMP);
...
MonitoredSample ManagerFastFlowPipeline::getSample() {
  ...
  for (KnobVirtualCoresFarm *fk : _farmsKnobs) {
    MonitoredSample tmp =
        getSampleResponse(fk->getActiveWorkers(), _samples->average().latency);
...
void ManagerFastFlowPipeline::postConfigurationManagement() {
  std::vector<AdaptiveNode *> newWorkers;
  for (size_t c = 0; c < _numHMP; c++) {
    const KnobVirtualCoresPipe *knobWorkers =
        dynamic_cast<const KnobVirtualCoresPipe *>(
            _configuration->getKnob(c, KNOB_VIRTUAL_CORES));
    auto tmp = knobWorkers->getActiveWorkers();
    newWorkers.insert(newWorkers.end(), tmp.begin(), tmp.end());
  }
  ...
  DEBUG("[Manager Pipeline] Moving from "
        << _activeWorkers.size() << " total workers to " << newWorkers.size()
        << " total workers.");
  if (_activeWorkers.size() != newWorkers.size()) {
