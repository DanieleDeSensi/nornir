...
void AdaptiveNode::getSampleResponse(MonitoredSample &sample,
                                     double avgLatency) {
...
void AdaptiveNode::askForSample() {
  ManagementRequest *request =
      &_managementRequests[MGMT_REQ_GET_AND_RESET_SAMPLE];
...
void AdaptiveNode::storeSample() {
  ...
  if (_numTasks) {
    _sampleResponse.latency =
        ((double) _ticksWork / (double) _numTasks) / _ticksPerNs;
  } else {
    _sampleResponse.latency = 0.0;
  }
  _sampleResponse.throughput =
      (double) _numTasks / ticksToSeconds(totalTicks, _ticksPerNs);
...
void AdaptiveNode::callbackIn(void *p) CX11_KEYWORD(final) {
  ...
  while (!_managementQ.empty()) {
    ...
    default: { throw runtime_error("Unexpected mgmt request."); } break;
    }
  }
}
...
void AdaptiveNode::disableRethreading() {
  if (_nodeType != NODE_TYPE_EMITTER) {
    throw std::runtime_error(
        "disableRethreading can only be called on the emitter.");
  }
