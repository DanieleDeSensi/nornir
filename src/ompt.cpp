 ...
 *
 * Contains hooks to OpenMP to let it communicate with a nornir external
 * manager.
 * ./bin/manger-external needs to be started before starting the OpenMP
 * application. Before starting the OpenMP application, simply LD_PRELOAD
 * nornir.
 *
 * =========================================================================
...
static inline void init() {
  if (omp_get_thread_num() == 0 && !initialized) {
    ...
    if (parameters_file) {
      tmpinstr = new nornir::Instrumenter(std::string(parameters_file),
                                          omp_get_num_threads(), NULL, true);
...
static void on_ompt_callback_task_schedule(ompt_data_t *first_task_data,
  ...
  DEBUG("[Nornir] %" PRIu64 ": ompt_event_task_schedule: first_task_id=%" PRIu64
        ", second_task_id=%" PRIu64 ", prior_task_status=%s=%d\n",
        ompt_get_thread_data()->value, first_task_data->value,
        second_task_data->value, ompt_task_status_t_values[prior_task_status],
        prior_task_status);
...
static void on_ompt_callback_chunk(unsigned long long chunk_size) {
