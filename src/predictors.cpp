...
#define DEBUG(x)                                                               \
  do {                                                                         \
    std::cerr << "[Predictors] " << x << std::endl;                            \
  } while (0)
#define DEBUGB(x)                                                              \
  do {                                                                         \
    x;                                                                         \
...
static double getVoltage(const VoltageTable &table, uint workers,
                         Frequency frequency) {
...
RegressionData::RegressionData(const Parameters &p,
  ...
  _phyCoresPerDomain = _p.mammut.getInstanceCpuFreq()
                           ->getDomains()
                           .at(0)
                           ->getVirtualCores()
                           .size() /
...
void RegressionDataServiceTime::init(const KnobsValues &values) {
  ...
  if (_p.knobFrequencyEnabled) {
    ...
    _invScalFactorSpeedAndCores =
        (double) _minSpeed / (numVirtualCores * frequency);
    ++_numPredictors;
    ...
    throw std::runtime_error(
        "Impossible to use Amdahl regression with no frequency.");
  }
}

RegressionDataServiceTime::RegressionDataServiceTime(
    const Parameters &p, const Configuration &configuration,
    const Smoother<MonitoredSample> *samples)
    : RegressionData(p, configuration, samples), _invScalFactorSpeed(0),
      _invScalFactorSpeedAndCores(0), _numPredictors(0) {
  _phyCores = _p.mammut.getInstanceTopology()->getPhysicalCores().size();
  std::vector<double> frequencies =
      _configuration.getKnob(KNOB_FREQUENCY)->getAllowedValues();
  if (!frequencies.empty()) {
    _minSpeed = frequencies[0];
    if (_p.knobClkModEnabled) {
      _minSpeed *=
          _configuration.getKnob(KNOB_CLKMOD)->getAllowedValues().front() /
          100.0;
    }
...
void RegressionDataServiceTime::toArmaRow(size_t columnId,
...
static void getStaticDynamicPower(double usedPhysicalCores, Frequency frequency,
                                  ...
                                  uint phyCoresPerDomain, uint &unusedDomains,
  ...
  if (mt == MAPPING_TYPE_LINEAR) {
    unusedDomains =
        numDomains - (usedPhysicalCores / (double) phyCoresPerDomain);
  ...
  for (uint x : coresPerCpu) {
    ...
    if (p.knobFrequencyEnabled) {
      voltage = getVoltage(*static_cast<mammut::cpufreq::VoltageTable *>(
                               p.archData.voltageTable),
                           phyCoresPerDomain, frequency);
    }
    staticPower += (voltage) *fullDomains;
    dynamicPower +=
        (phyCoresPerDomain * frequency * voltage * voltage * clkMod) *
        fullDomains;
    ...
    if (coresOnSpuriousDomain) {
      if (p.knobFrequencyEnabled) {
        voltage = getVoltage(*static_cast<mammut::cpufreq::VoltageTable *>(
                                 p.archData.voltageTable),
                             coresOnSpuriousDomain, frequency);
      }
      staticPower += voltage;
      dynamicPower +=
          coresOnSpuriousDomain * frequency * voltage * voltage * clkMod;
    }
  }
}
...
void RegressionDataPower::init(const KnobsValues &values) {
  ...
  if (_p.knobFrequencyEnabled || _p.knobClkModEnabled) {
    ...
    getStaticDynamicPower(usedPhysicalCores, frequency,
                          (MappingType) values[KNOB_MAPPING], clkMod, _p, _cpus,
                          _domains, _phyCoresPerCpu, _phyCoresPerDomain,
    ...
    default: {
      throw std::runtime_error(
          "RegressionDataPower: init: strategyUnused unsupported.");
    }
    }
    double voltage = 1.0;
    if (_p.knobFrequencyEnabled) {
      voltage = getVoltage(*static_cast<mammut::cpufreq::VoltageTable *>(
                               _p.archData.voltageTable),
                           0, frequencyUnused);
    }
    // See TACO2016 paper, equations 6. and 7.
    _voltagePerUnusedDomains =
        (voltage * unusedDomains) + staticPowerUsedDomains;
    ++_numPredictors;
...
RegressionDataPower::RegressionDataPower(
    const Parameters &p, const Configuration &configuration,
    const Smoother<MonitoredSample> *samples)
...
Predictor::Predictor(PredictorType type, const Parameters &p,
                     const Configuration &configuration,
                     const Smoother<MonitoredSample> *samples)
    : _type(type), _p(p), _configuration(configuration), _samples(samples),
      _modelError(0) {
  ;
}
...
PredictorLinearRegression::PredictorLinearRegression(
    PredictorType type, const Parameters &p, const Configuration &configuration,
    const Smoother<MonitoredSample> *samples)
...
void PredictorLinearRegression::clear() {
  for (obs_it iterator = _observations.begin(); iterator != _observations.end();
       ++iterator) {
...
void PredictorLinearRegression::refine() {
  ...
  DEBUG("Refining with configuration " << currentValues << ": " << response);
...
void PredictorLinearRegression::prepareForPredictions() {
  if (_preparationNeeded) {
    ...
    for (obs_it iterator = _observations.begin();
         iterator != _observations.end(); ++iterator) {
...
double PredictorLinearRegression::getInactivePowerParameter() const {
  if (_type == PREDICTION_POWER) {
    size_t pos;
    if (!dynamic_cast<RegressionDataPower *>(_predictionInput)
             ->getInactivePowerPosition(pos)) {
      throw std::runtime_error("Impossible to get inactive power parameter.");
    }
    return _lr.Parameters().at(pos);
  } else {
    throw std::runtime_error(
        "getInactivePowerParameter can only be called on POWER predictors.");
  }
}
#endif
...
#ifdef ENABLE_GSL
/**************** PredictorUSL ****************/
PredictorUsl::PredictorUsl(PredictorType type, const Parameters &p,
                           const Configuration &configuration,
                           const Smoother<MonitoredSample> *samples)
    : Predictor(type, p, configuration, samples),
      _maxPolDegree(POLYNOMIAL_DEGREE_USL), _ws(NULL), _x(NULL), _y(NULL),
      _chisq(0), _preparationNeeded(true), _maxSpeedThr(0), _minSpeedThr(0),
      _minSpeedCoresThr(0), _minSpeedCoresThrNew(0), _n1(0), _n2(0),
      _minSpeedN1Thr(0), _minSpeedN2Thr(0) {
  if (type != PREDICTION_THROUGHPUT) {
    throw std::runtime_error(
        "PredictorUsl can only be used for throughput predictions.");
  }
  ...
  if (_p.knobClkModEnabled) {
    _minSpeed *=
        _configuration.getKnob(KNOB_CLKMOD)->getAllowedValues().front() / 100.0;
    _maxSpeed *=
        _configuration.getKnob(KNOB_CLKMOD)->getAllowedValues().back() / 100.0;
  }
}
...
void PredictorUsl::refine() {
  ...
  if (speed == _maxSpeed && numCores == maxCores) {
    ...
    // We get the expected throughput at minimum frequency starting from the
    // actual throughput at generic frequency. To do so we apply the inverse of
    // the function used in predict() to get the throughput at a generic
    // frequency starting from that at minimum frequency.
    throughput = (throughput * (_maxSpeed - _minSpeed)) /
                 (_maxSpeed - speed + minMaxScaling * (speed - _minSpeed));
    // speed = _minSpeed;
  }
...
double PredictorUsl::predict(const KnobsValues &knobsValues) {
  ...
  if (speed != _minSpeed) {
    ...
    return ((maxFreqPred - throughput) / (_maxSpeed - _minSpeed)) * speed +
           ((throughput * _maxSpeed) - (maxFreqPred * _minSpeed)) /
               (_maxSpeed - _minSpeed);
  } else {
    return throughput;
  }
...
double PredictorUsl::getTheta(RecordInterferenceArgument n) {
  ...
  if (_p.knobClkModEnabled) {
    throw std::runtime_error(
        "Interference cannot be updated when clock modulation is used.");
  }
  return (((getMinSpeedCoresThr() * numCores) / predict(kv)) - 1) -
         (((_minSpeedCoresThrNew * numCores) / throughput) - 1);
}
...
PredictorAnalytical::PredictorAnalytical(
    PredictorType type, const Parameters &p, const Configuration &configuration,
    const Smoother<MonitoredSample> *samples)
    : Predictor(type, p, configuration, samples) {
...
double PredictorAnalytical::getScalingFactor(const KnobsValues &values) {
  double ratioCores = values[KNOB_VIRTUAL_CORES] /
                      _configuration.getRealValue(KNOB_VIRTUAL_CORES);
  double ratioClkMod =
      values[KNOB_CLKMOD] / _configuration.getRealValue(KNOB_CLKMOD);
  double ratioFrequency =
      values[KNOB_FREQUENCY] / _configuration.getRealValue(KNOB_FREQUENCY);
...
PredictorAnalyticalFull::PredictorAnalyticalFull(
    PredictorType type, const Parameters &p, const Configuration &configuration,
    const Smoother<MonitoredSample> *samples)
  ...
  case PREDICTION_POWER: {
    if (_configuration.equal(values)) {
      ...
      double scalingFactor =
          getPowerPrediction(_configuration.getRealValues()) /
          (getCurrentPower() - _p.archData.idlePower);
      return _p.archData.idlePower +
             (getPowerPrediction(values) / scalingFactor);
    }
  } break;
  }
  return 0.0;
...
PredictorLeo::PredictorLeo(PredictorType type, const Parameters &p,
                           const Configuration &configuration,
                           const Smoother<MonitoredSample> *samples)
    : Predictor(type, p, configuration, samples), _preparationNeeded(true) {
  ...
  if (!appFound) {
    throw std::runtime_error("Impossible to find application " +
                             p.leo.applicationName + " in " + p.leo.namesData);
  }
}
...
void PredictorLeo::refine() {
  ...
  if (it == _confIndexes.end()) {
    throw std::runtime_error(
        "[Leo] Impossible to find index for configuration.");
  }
  ...
  default: { throw std::runtime_error("[Leo] Unknown predictor type."); }
  }
}
...
void PredictorLeo::prepareForPredictions() {
  if (_preparationNeeded) {
    ...
    leo::PredictionResults pr =
        leo::compute(_appId, dataFile, &_values, perColumnNormalization);
...
PredictorSMT::PredictorSMT(PredictorType type, const Parameters &p,
                           const Configuration &configuration,
                           const Smoother<MonitoredSample> *samples)
    : Predictor(type, p, configuration, samples) {
...
void PredictorSMT::addObservation(mat &_xs, rowvec &_ys, const vec &x,
                                  const double y) {
  int col = -1;
...
void PredictorSMT::refine() {
  double numContexts =
      _configuration.getKnob(KNOB_HYPERTHREADING)->getRealValue();
  double numCores =
  ...
  case PREDICTION_POWER: {
    ...
    double fullCPUvoltage = getVoltage(
        *static_cast<mammut::cpufreq::VoltageTable *>(_p.archData.voltageTable),
        _phyCoresPerDomain, freq);
    double spuriousCPUVoltage = getVoltage(
        *static_cast<mammut::cpufreq::VoltageTable *>(_p.archData.voltageTable),
        nCoresInSpuriousCpu, freq);

    vec x(4);
    x(0) = (_domains - nActiveCpu) *
           getVoltage(*static_cast<mammut::cpufreq::VoltageTable *>(
                          _p.archData.voltageTable),
                      0, freq); // Static power of inutilized domains
    x(1) = nActiveCpu * fullCPUvoltage +
           spuriousCPUVoltage; // Static power of utilized domains
    x(2) = nActiveCpu * fullCPUvoltage * fullCPUvoltage * freq *
               _phyCoresPerDomain +
           spuriousCPUVoltage * spuriousCPUVoltage * freq *
               nCoresInSpuriousCpu; // Dynamic power of utilized domains
...
void PredictorSMT::prepareForPredictions() {
  if (_preparationNeeded) {
	...
	delete _lr1;
      ...
      mat ht =
          _xs2.submat(_xs2.n_rows - 1, 0, _xs2.n_rows - 1, _xs2.n_cols - 1);
...
double PredictorSMT::predict(const KnobsValues &knobValues) {
  ...
  case PREDICTION_POWER: {
    ...
    mat p_regr(4, 1);
    p_regr(0, 0) = (_domains - nActiveCpu) *
                   getVoltage(*static_cast<mammut::cpufreq::VoltageTable *>(
                                  _p.archData.voltageTable),
                              0, freq);
    p_regr(1, 0) = nActiveCpu * fullCPUvoltage + spuriousCPUVoltage;
    p_regr(2, 0) =
        nActiveCpu * fullCPUvoltage * fullCPUvoltage * freq *
            _phyCoresPerDomain +
        spuriousCPUVoltage * spuriousCPUVoltage * freq * nCoresInSpuriousCpu;
    ...
    DEBUG("Predict power for " << numCores << " " << numContexts << " " << freq
                               << " --> " << power_ht(0));
    return power_ht(0);
